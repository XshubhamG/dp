...
export default function HomePage() {
  ...
  const [backgroundInput, setBackgroundInput] =
    React.useState(DEFAULT_BACKGROUND);
  ...
  return (
    <div className="min-h-screen bg-(--background)">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <h1 className="text-2xl font-semibold">
              Create gentle profile initials
            </h1>
          </div>
        </div>
        <ThemeToggle />
      ...
      <main className="mx-auto grid min-h-screen max-w-6xl place-content-center gap-10 px-6 pb-16 md:grid-cols-[1.1fr_0.9fr] md:px-12">
        <section className="space-y-6 rounded-[32px] border border-[var(--border)] bg-[var(--surface)] p-6 shadow-sm">
          <div className="space-y-2">
            ...
            <p className="text-sm text-[var(--muted-foreground)]">
              Serve a personalized SVG avatar based on the name and your chosen
              background color.
            </p>
          </div>
        ...
        <section className="flex flex-col gap-6 rounded-4xl border border-(--border) bg-(--surface) p-6 shadow-sm">
          ...
          <div className="flex flex-1 items-center justify-center rounded-3xl border border-(--border) bg-(--muted) p-8">
            <Image
              src={avatarUrl}
              alt={`Avatar for ${name}`}
              width={192}
          ...
          <div className="rounded-3xl border border-(--border) bg-(--muted) p-4 text-sm text-(--muted-foreground)">
            <p className="font-medium text-(--foreground)">API parameters</p>
